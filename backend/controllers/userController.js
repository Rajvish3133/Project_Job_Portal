    ...
    try {
     const{
       ...
       if(role==="Job Seeker" && (!firstNiche || !secondNiche || !thirdNiche)){
        return next(new ErrorHandler("please provide your prferred job niches"));
       }
...
export const login = catchAsyncErrors(async(req, res, next)=>{
 const{ role, email, password } = req.body;
  ...
  if(!isPasswordMatched){
    ...
    if(user.role !== role){
        return next(new ErrorHandler("Incorrect role selected for this email", 400));
    } 
...
export const updateProfile = catchAsyncErrors(async(req,res,next)=>{
    ...
    if(req.user.role === "Job Seeker" && (!firstNiche || !secondNiche || !thirdNiche)){
        return next(new ErrorHandler("please provide your all prferred job niches",400));
       } 
