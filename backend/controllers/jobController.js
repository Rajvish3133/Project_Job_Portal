...
export const postJob = catchAsyncErrors(async(req,res,next)=>{
    ...
    const {
     ...
     if(!title || !jobType || !location || !companyName || !introduction || !responsibilities || !qualifications || !salary || !jobNiche){
        return next(new ErrorHandler("Please provides full job details",400));
     }
  if((personalWebsiteTitle && !personalWebsiteUrl)||(personalWebsiteUrl && !personalWebsiteTitle)   ){
    return next(new ErrorHandler("provide both the website url and title , or leave both empty",400));
  }
